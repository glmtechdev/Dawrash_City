...
const navItems: {
  ...
  { value: 'members', label: 'Members Directory', icon: Users },
  {
    value: 'transactions',
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="border-b border-border bg-card md:hidden">
          <div className="flex items-center justify-between px-4 py-3">
            <Brand subtitle />
            <Badge className="border-gold/30 bg-gold/10 text-gold text-xs">Superadmin</Badge>
          </div>
          <nav className="flex gap-1.5 overflow-x-auto px-3 pb-3 scrollbar-none">
            {navItems.map((item) => {
              const active = section === item.value
              const badgeCount = item.badgeKey ? counts[item.badgeKey] ?? 0 : 0
              return (
                <button
                  ...
                  onClick={() => onSection(item.value)}
                  className={cn(
                    'flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full px-3.5 py-1.5 text-xs font-medium transition-colors',
                    active
                ...
                >
                  <span>{item.label}</span>
                  {badgeCount > 0 && (
                    <span className="inline-flex size-4 items-center justify-center rounded-full bg-background/25 text-[10px] font-bold">
                      {badgeCount}
                    </span>
                  )}
                </button>
